...
import { featuredPublication } from '@/lib/site';
import { buttonClassName } from '@/components/ui/button';

export const metadata: Metadata = {
  title: featuredPublication.title,
  description: featuredPublication.abstract
};
  ...
  return (
    <section className="mx-auto w-full max-w-5xl px-5 py-16 md:px-8 md:py-20">
      ...
      <article className="mt-8 rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur-xl md:p-10">
        ...
        <h1 className="mt-3 text-4xl font-semibold text-white md:text-5xl">{featuredPublication.title}</h1>
        <p className="mt-3 text-sm text-slate-300">{featuredPublication.journal} ({featuredPublication.year})</p>

        <div className="mt-8 rounded-3xl border border-white/10 bg-black/20 p-6">
          <p className="text-xs uppercase tracking-[0.35em] text-sky-300/75">Abstract Preview</p>
          <p className="mt-4 text-sm leading-7 text-slate-300">{featuredPublication.abstract}</p>
        </div>

        <div className="mt-6 grid gap-3 md:grid-cols-3">
          {featuredPublication.points.map((point) => (
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          <Link href={featuredPublication.readUrl} className={buttonClassName('primary')}>
